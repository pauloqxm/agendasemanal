...
# =========================
# Estilo (cinza claro + azul nos detalhes)
# =========================
def apply_css():
    st.markdown(
        """
        <style>
          :root{
            ...
            --brand:#1E40AF;   /* azul escuro */
            --brand2:#3B82F6;  /* azul vivo */
            --shadow: 0 16px 45px rgba(2,6,23,0.10);
          }

          .stApp{
            background:
              radial-gradient(1100px 520px at 15% 0%, rgba(59,130,246,0.10), transparent 60%),
              radial-gradient(900px 520px at 100% 15%, rgba(30,64,175,0.10), transparent 55%),
          ...
          [data-testid="stSidebar"]{
            background: linear-gradient(180deg, var(--panel) 0%, #F7F9FF 100%);
          ...
          .stButton > button{
            ...
            box-shadow: 0 14px 34px rgba(30,64,175,0.20);
          ...
          .topbar{
            background: linear-gradient(135deg, rgba(255,255,255,0.92), rgba(250,251,255,0.90));
            border: 1px solid var(--stroke);
            border-radius: 22px;
            padding: 14px 16px;
            margin-bottom: 12px;
          ...
          .topbar:before{
            ...
            inset:0;
            background:
              radial-gradient(500px 160px at 10% 0%, rgba(59,130,246,0.18), transparent 60%),
              radial-gradient(520px 160px at 100% 10%, rgba(30,64,175,0.14), transparent 55%);
            pointer-events:none;
          }
          .topbar-row{
            position: relative;
          ...
          /* “Abas” no topo (segmented control) */
          ...
          /* deixa o item selecionado com cara de aba azul */
...
def page_cadastrar_evento():
    ...
    with col1:
        if ev and val("congregacao") in allowed_congregs:
            ...
            congregacao = st.selectbox("🏛️ Congregação*", allowed_congregs, index=None, placeholder="Selecione a congregação")

    with col2:
        if ev and val("tipo") in TIPOS:
            tipo = st.selectbox("📌 Tipo da agenda*", TIPOS, index=TIPOS.index(val("tipo")))
        else:
            tipo = st.selectbox("📌 Tipo da agenda*", TIPOS, index=None, placeholder="Escolha o tipo do evento")
